...
class CelebA(datasets.VisionDataset):
    ...
    def _load_data(self):
        ...
        with open(os.path.join(
                self.root, self.base_folder, "Eval/list_eval_partition.txt")) as csv_file:
            data = list(csv.reader(csv_file, delimiter=" ", skipinitialspace=True))
        with open(os.path.join(
            self.root, self.base_folder, "Anno/list_attr_celeba.txt")) as csv_file:
    ...
    def __getitem__(self, index):
        X = PIL.Image.open(os.path.join(  # noqa
            self.root, self.base_folder, "Img/img_align_celeba", self.filename[index]))
