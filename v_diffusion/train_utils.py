...
class Trainer:
    ...
    def step(self, x, y, update=True, use_encoder=True):
        # print(id(self.z_model))
        # print(id(self.model.encoder))
        B = x.shape[0]
        ...
        if update:
            ...
            if use_encoder is True:
                nn.utils.clip_grad_norm_(self.z_model.parameters(), max_norm=self.grad_norm)
        ...
        with self.ema:
            if batch_size == -1:
                sample = diffusion.p_sample(
                    denoise_fn=self.model, shape=shape, x_start=x_start, device=self.device,
        ...
        for e in range(self.start_epoch, self.epochs):
            ...
            if self.is_main:
                if not (e + 1) % self.image_intv and self.num_save_images and image_dir:
                    x = self.sample_fn_enc(
                        noises, labels=None, x_start=x_0, use_ddim=use_ddim, batch_size=sample_bsz)
    ...
    def load_checkpoint(self, chkpt_path, map_location):
        chkpt = torch.load(chkpt_path, map_location=map_location)
        getattr(self, "z_optimizer").load_state_dict(chkpt["z_optimizer"])
        getattr(self, "z_model").load_state_dict(chkpt["z_model"])
        for trainee in self.trainees:
            try:
                trainee_model = trainee                   
    ...
    # def load_checkpoint_temp(self, chkpt_path, map_location):
    #     chkpt = torch.load(chkpt_path, map_location=map_location)
    #     getattr(self, "student_model").load_state_dict(chkpt["G"])
    #     # getattr(self, "student_optimizer").load_state_dict(chkpt["scheduler"])
    #     # getattr(self, "z_optimizer").load_state_dict(chkpt["optimizerG"])
    #     self.start_epoch = 0

    def save_checkpoint(self, chkpt_path, **extra_info):
        chkpt = []
        chkpt.append(("z_model", getattr(self, "z_model").state_dict()))
        chkpt.append(("z_optimizer", getattr(self, "z_optimizer").state_dict()))
        for k, v in self.named_state_dicts():
            chkpt.append((k, v))
        for k, v in extra_info.items():
            chkpt.append((k, v))
...
class DistillationTrainer:
    ...
    def distill_step(self, x, y, update=True,use_encoder=True):
        B = x.shape[0]
        loss = self.loss(x, y).mean()
        loss.div(self.num_accum).backward()
        if update:
    ...
    def sample_fn_enc(
        ...
        with self.ema:
            if batch_size == -1:
                sample = diffusion.p_sample(
                    denoise_fn=self.student_model, shape=shape, x_start=x_start, device=self.device,
                    noise=noises, label=labels, use_ddim=use_ddim)
            else:
                sample = []
                for i in range(0, noises.shape[0], batch_size):
    ...
    def train_G(
        ...
        N = 0
        L_tot = 0
        self.z_model = self.student_model.module.encoder
        # self.z_model = self.student_model.encoder
        for e in range(self.start_epoch, self.epochs):
            self.stats.reset()
