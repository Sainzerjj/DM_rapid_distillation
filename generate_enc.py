if __name__ == "__main__":
    ...
    from v_diffusion.utils import save_image_cuda
    ...
    parser.add_argument("--total-size", default=202500, type=int)
    parser.add_argument("--config-dir", default="./configs", type=str)
    parser.add_argument("--chkpt-dir", default="./checkpoints", type=str)
    parser.add_argument("--chkpt-path", default="./checkpoints/teacher/celeba_v_diffusion_enc_250.pt", type=str)
    ...
    parser.add_argument("--sample-timesteps", default=10, type=int)
    parser.add_argument("--uncond", default=True)   # action="store_true"
    parser.add_argument("--w-guide", default=0.3, type=float)
    parser.add_argument("--suffix", default="hh", type=str)
    parser.add_argument("--seed", default=12, type=int)
    parser.add_argument("--task", default="fid", choices=["fid", "single_sample"], type=str)
    # action="store_true"
    ...
    # ])
    # ds = torchvision.datasets.CIFAR10(root='./data', download=True, transform=transform)
    # dataloader = torch.utils.data.DataLoader(ds, batch_size, shuffle=False)
    dataloader, sampler = get_dataloader(
        dataset, batch_size, split="all", val_size=0., random_seed=args.seed,
        root=root, drop_last=True, pin_memory=True) 
    if args.task == "single_sample":
        seed_all(args.seed)
        x = diffusion.p_sample(
                    model, shape=shape, x_start=None, device=device,
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as pool:
            for i, (x_start, y) in enumerate(tqdm(dataloader)):
                x_start = x_start.to(device)
                if i == num_eval_batches - 1:
                    batch_size = total_size - i * batch_size
                    shape = (batch_size, 3, image_res, image_res)
                x = diffusion.p_sample(
                    model, shape=shape, x_start=x_start, device=device,
                    noise=torch.randn(shape, device=device),
                    label=None, # next(label_loader)[:batch_size],
                    use_ddim=args.use_ddim
                )
                evaluator.istats(x.to(device))
                to_range_0_1 = lambda x: (x + 1.) / 2.
                save_image_cuda(x, "result_pred_x.png", nrow=8, normalize=True, value_range=(-1., 1.))
