...
class BeatGANsUNetConfig(BaseConfig):
    ...
    #######################################
    sel_attn_depth: int = 8
    sel_attn_block: str = "output"
...
class BeatGANsUNetModel(nn.Module):
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        if conf.num_classes is not None:
            ...
            #         OneHot(conf.num_classes, exclude_zero=True),
            #         Linear(conf.num_classes, conf.embed_channels)
            #     )
        ...
        for level, mult in list(enumerate(conf.channel_mult))[::-1]:
            for i in range(conf.num_res_blocks + 1):
                ...
                self._feature_size += ch
            # print('outputs:', self.output_blocks)
        # print(input_block_chans)
        ...
        if conf.sel_attn_block == "middle":
            self.extract_attention = self.middle_block[1].attention
        elif conf.sel_attn_block == "output":
            assert conf.sel_attn_depth <= 8 and conf.sel_attn_depth >= 0, "sel_attn_depth must be between 0 and 8"
            self.extract_attention = self.output_blocks[conf.sel_attn_depth][1].attention
        else:
