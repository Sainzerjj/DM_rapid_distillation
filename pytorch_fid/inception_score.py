...
def get_inception_score(images, splits=10):
    ...
    return preds, mean, std  # Reference values: 11.38 for 50000 CIFAR-10 training set images, or mean=11.31, std=0.10 if in 10 splits.


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--sample_dir', default='./images/cifar10/samples/', help='path to saved images')
    opt = parser.parse_args()

    data = np.load(opt.sample_dir)
    data = np.clip(data, 0, 255)
    p, m, s = get_inception_score(data, splits=1)
